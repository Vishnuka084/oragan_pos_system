...
public class OderDetailsUpdateFormController {
    ...
    public RadioButton radBtnCredit;
    public RadioButton radBtnDebite;
    ...
    public void setOrderDetails(Order order) {
        ...
        // set radio buttons credit debit card option
        if (order.getCreditOrDebit().equals("Credit")) {
            radBtnCredit.setSelected((true);
        } else if (order.getCreditOrDebit().equals("Debit")){
            radBtnDebite.setSelected(true);
        }
...
public class OderDetailsUpdateFormController {
    ...
    public void setOrderDetails(Order order) {
        ...
        // Set radio buttons for credit/debit card option
        if (order.getCreditOrDebit().equals("Credit")) {
            radBtnCredit.setSelected(true);
        } else if (order.getCreditOrDebit().equals("Debit")) {
            radBtnDebite.setSelected(true);
    ...
    //------------- Order Details Save ------------------
    public void btnOrderDetailsSaveAddOnAction(ActionEvent actionEvent) {
        if (currentOrder != null) {
            try {
                updateOrderDetails();
                showAlert(Alert.AlertType.INFORMATION, "Success", "Order details updated successfully.");
    ...
    private void updateOrderDetails() throws SQLException, ClassNotFoundException {
        ...
        String status = radBtnHold.isSelected() ? "Hold" : radBtnConfirm.isSelected() ? "Confirmed" : "";
        String issueDate = issueDateId.getValue().toString();
        String creditOrDebit = radBtnCredit.isSelected() ? "Credit" : radBtnDebite.isSelected() ? "Debit" : "";

        String updateQuery = "UPDATE `Order` SET Customer_Id = ?, Customer_name = ?, Current_Date = ?, Total = ?, Status = ?, Issue_Date = ?, Credit_Or_Debit = ? WHERE OrderID = ?";
